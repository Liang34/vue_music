...
export const sequencePlay = function ({ commit }, { list }) {
  commit(types.SET_PLAY_MODE, playMode.sequence)// 更新播放模式
  commit(types.SET_SEQUENCE_LIST, list)// 更新乱序播放
  commit(types.SET_PLAYLIST, list)// 更新歌曲列表
  commit(types.SET_CURRENT_INDEX, 0)// 更新播放歌曲索引
  commit(types.SET_FULL_SCREEN, true)// 设置全屏显示
  commit(types.SET_PLAYING_STATE, true)// 歌曲改为播放状态
}
// 点击歌曲播放
